...
- **Tuning loop (steps 3+): DONE 2026-06-23, ACCEPTED + SHIPPED.** Ran the A/D/C/B loop autonomously (Kevin's mandate). Outcome = "C2": knob A (`carnivoreEatGain 40`) + knob D (prey-dependent carnivore rescue) + knob C (`herbivoreCrowding 2.0`, herbivore conspecific crowding = spatial dispersion). On the 1000-tick protocol, C2 vs the seeded baseline: extinction 17%->0%, carnivore-persistence 0%->83%, phase lag -44t->+68t, cap-hits 0. Knob C (dispersion) was the load-bearing lever as predicted. Knob B (local density-dependent birth) tested and REJECTED (inert under C). Long-run (3000t) is limited by the PARADOX OF ENRICHMENT under the non-stationary world (K rises without bound -> cycle amplitude grows -> some seeds crash); accepted as organic living-world behavior, `faunaMaxPop` kept at 400 as a benign backstop. Full A/B trail: `docs/01 Design/Ecosystem Tuning Log.md`. Durable lessons (RNG reshuffle, eatGain-vs-eatSpeed, crowding-is-load-bearing, paradox-of-enrichment): `docs/02 Code/Engineering Lessons.md`. If steady-state long-run balance is later wanted, bound K generation-side or add an enrichment-robust mechanism (predator interference / prey predation-refuge).
