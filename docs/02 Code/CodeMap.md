...
- **Rail tooltips (fixed 2026-08-21):** the `data-tip` system is a delegated IIFE in `main.js` bound to `document.querySelectorAll('.deck, .hand-rail')`. The rail had to be added explicitly because it lives in `.canvas-wrap` and is not a `.deck`, so its ten tooltips never fired before this. Rail tips position BESIDE the hovered button (`el.closest('.hand-rail')` branch); deck tips still position below.
- **What Viewer actually reaches:** `#handLaws` reveals the WHOLE sidebar, not just the dials - Chronicle, Legend, Inspector, Species, Lineage, Food Web and Population as well as Terrain/Climate/Ecology. Hover reporting (`updateTooltip` -> `#tip`) also works in Viewer with no hiding rule. Still unreachable in Viewer in both drawer states: `#btnRollSeed`, `#btnCopyLink`. `#btnReset` was unhidden 2026-08-21 and, in Viewer only, resumes the loop after rebuilding.
- **Narrow screens (added 2026-08-21):** one `@media (max-width:700px)` block at the END of the `index.html` style block is the only responsive CSS in the project. It stacks `.canvas-wrap` into a column, turns `.hand-rail` into a horizontal bar (kept `position:relative`, NOT static, so the flyout still anchors to it), flips `.hand-flyout` above the rail, and sheds the deck's optional chrome so its intrinsic width fits a phone. `fitCanvas` pairs with it: it subtracts 56px of height under 700px and floors `PIX` at 2 rather than 6.
