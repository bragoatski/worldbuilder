...
## NEXT (in order)
1. **Ecosystem tuning loop - START with knob A** (awaiting Kevin's go per step): knob A (predator numerical-response lag - the "do not grow too fast" knob) -> add D (spawn rescue) -> tune C (dispersion/asynchrony) -> fine-tune B alongside. Each step one reproducible A/B vs the seeded baseline, kept only if the cycle metrics improve. Stop when the Decision-1 target (bounded coexisting oscillation, no cap-hits) is met. The instrument is ready: the harness now SHOWS the cycle (phase lag etc.), so each knob change is judged by the metrics, not eyeballed. (If you switch the loop to `--snapshot` for speed, re-baseline under snapshot mode first - see the Engineering Lessons caveat.)
